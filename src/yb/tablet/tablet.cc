...
Status Tablet::UpdateIndexBackfillOrderingGeneration(
    const OpId& op_id, const TableId& table_id, bool activate, HybridTime retention_barrier_ht,
    uint32_t write_id_floor_version) {
...
Result<docdb::UniqueIndexVerificationResult> Tablet::VerifyUniqueIndex(
  ...
  auto doc_db = this->doc_db();
  auto result = VERIFY_RESULT(docdb::VerifyUniqueIndexTablet(
      doc_db.regular, *doc_db.key_bounds, metadata_.get(), options));
