...
class BackfillTable : public std::enable_shared_from_this<BackfillTable> {
 ...
 private:
  ...
  Status LaunchShadowVerificationOrFinish();
  Status StartShadowVerificationForNextIndex() EXCLUDES(mutex_);
  Status LaunchShadowVerificationTablet(const TabletInfoPtr& tablet, const std::string& start_key);
...
class UpdateOrderingGenerationForTablet : public RetryingTSRpcTaskWithTable {
 ...
 private:
  ...
  const bool notify_backfill_table_;
};
