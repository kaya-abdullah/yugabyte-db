...
UniqueIndexBackfillMode SelectUniqueIndexBackfillMode() {
  const auto& test_mode = FLAGS_TEST_ysql_index_backfill_unique_check_mode;
  if (PREDICT_FALSE(!test_mode.empty())) {
...
Result<std::vector<scoped_refptr<TableInfo>>> BackfillTable::GetUniqueIndexTables(
    bool restrict_to_indexes_to_build) const {
...
Status BackfillTable::LaunchOrderingGenerationActivation() {
  ...
  auto index_tables = VERIFY_RESULT(GetUniqueIndexTables(/* restrict_to_indexes_to_build= */
                                                         true));
  ...
  for (auto& [tablet, index_table_id] : tablets) {
    auto task = std::make_shared<UpdateOrderingGenerationForTablet>(
        shared_from_this(), tablet, index_table_id, /* activate= */ true,
        /* notify_backfill_table= */ true, epoch_);
...
Status BackfillTable::SendRpcToReleaseOrderingGenerations() {
  ...
  for (const auto& index_table : index_tables) {
    for (const auto& tablet : VERIFY_RESULT(index_table->GetTablets())) {
      auto task = std::make_shared<UpdateOrderingGenerationForTablet>(
          shared_from_this(), tablet, index_table->id(), /* activate= */ false,
          /* notify_backfill_table= */ false, epoch_);
...
Status BackfillTable::LaunchShadowVerificationOrFinish() {
  ...
  auto index_tables_result = GetUniqueIndexTables(/* restrict_to_indexes_to_build= */ false);
...
// M3 of the coordinator review: the shadow phase is on the publication critical path
// (CREATE INDEX waits for it), so no coordinator failure may strand the job short of the
// terminal funnel. Best-effort-record INCONCLUSIVE, then always continue into publication.
