 ...
 protected:
  ...
  std::string PackedRow(
    ...
    if (version == dockv::PackedRowVersion::kV1) {
      ...
      EXPECT_OK(packer.AddValue(basectid_column_id_, BinaryValue(identity)));
      EXPECT_OK(packer.AddValue(include_column_id_, include_value));
      return EXPECT_RESULT(packer.Complete()).ToBuffer();
    }
